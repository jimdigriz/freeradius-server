...
static CONF_PARSER module_config[] = {
	...
	{ FR_CONF_OFFSET("lua_preamble", FR_TYPE_FILE_INPUT | FR_TYPE_REQUIRED, rlm_redis_ippool_t, lua_preamble_file), .dflt = "${modconfdir}/redis/ippool/preamble.lua" },
	{ FR_CONF_OFFSET("lua_alloc", FR_TYPE_FILE_INPUT | FR_TYPE_REQUIRED, rlm_redis_ippool_t, lua_alloc.file), .dflt = "${modconfdir}/redis/ippool/alloc.lua" },
	{ FR_CONF_OFFSET("lua_update", FR_TYPE_FILE_INPUT | FR_TYPE_REQUIRED, rlm_redis_ippool_t, lua_update.file), .dflt = "${modconfdir}/redis/ippool/update.lua" },
	{ FR_CONF_OFFSET("lua_release", FR_TYPE_FILE_INPUT | FR_TYPE_REQUIRED, rlm_redis_ippool_t, lua_release.file), .dflt = "${modconfdir}/redis/ippool/release.lua" },
...
static int expires_map(rlm_redis_ippool_t const *inst, REQUEST *request, unsigned long expires)
{
	if (!inst->expiry_attr)
		return 0;
	...
	expiry_rhs.tmpl_value_type = FR_TYPE_UINT32;
	expiry_rhs.tmpl_value.vb_uint32 = (uint32_t) expires;
	vp_map_t expiry_map = {
		.lhs = inst->expiry_attr,
		.op = T_OP_SET,
		.rhs = &expiry_rhs
	...
	if (map_to_request(request, &expiry_map, map_to_vp, NULL) < 0) {
		RPEDEBUG("Failed mapping expires to %s", inst->expiry_attr->name);
		return -1;
	}
...
static rlm_rcode_t mod_action(rlm_redis_ippool_t const *inst, REQUEST *request, ippool_action_t action)
{
	...
	case POOL_ACTION_ALLOCATE:
		...
		case IPPOOL_RCODE_SUCCESS:
			...
			if (expires_map(inst, request, expires))
				return RLM_MODULE_FAIL;
	...
	case POOL_ACTION_UPDATE:
	{
		...
		case IPPOOL_RCODE_SUCCESS:
			...
			if (expires_map(inst, request, expires) < 0)
				return RLM_MODULE_FAIL;
...
static int mod_instantiate(void *instance, CONF_SECTION *conf)
{
	...
	if (redis_ippool_loadscript(conf, &lua_preamble, &inst->lua_alloc))
		goto err2;
	if (redis_ippool_loadscript(conf, &lua_preamble, &inst->lua_update))
		goto err2;
	if (redis_ippool_loadscript(conf, &lua_preamble, &inst->lua_release))
		goto err2;
