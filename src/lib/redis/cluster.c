...
int redis_ippool_loadscript_buf(TALLOC_CTX *ctx, redis_ippool_lua_script_t const *preamble, redis_ippool_lua_script_t *script)
{
	FILE		*f;
	struct stat	finfo;
	int		len;
...
int redis_ippool_loadscript(TALLOC_CTX *ctx, redis_ippool_lua_script_t const *preamble, redis_ippool_lua_script_t *script)
{
	int len;
	fr_sha1_ctx	sha1_ctx;
	uint8_t		digest_bin[SHA1_DIGEST_LENGTH];

	if ((len = redis_ippool_loadscript_buf(ctx, preamble, script)) == -1)
		return -1;
 ...
 * @param[in] cmd		EVALSHA command to execute (must begin with "EVALSHA ")
 * @param[in] ...		Arguments for the EVALSHA command (first argument must be digest)
					...
					char const *script, char const *cmd, ...)
{
	...
	rad_assert(talloc_array_length(script) > 0);
	...
	rad_assert(talloc_array_length(digest) == 2 * SHA1_DIGEST_LENGTH + 1);

	rad_assert(strlen(cmd) >= 10);
	rad_assert(strncmp(cmd, "EVALSHA %s", 10) == 0);
